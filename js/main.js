...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCarousel() {
    ...
    slides.forEach((slide, idx) => {
      ...
      if (idx === currentIndex) slide.classList.add('current');
      if (idx === (currentIndex - 1 + slides.length) % slides.length) slide.classList.add('prev');
      if (idx === (currentIndex + 1) % slides.length) slide.classList.add('next');
    });
  }
